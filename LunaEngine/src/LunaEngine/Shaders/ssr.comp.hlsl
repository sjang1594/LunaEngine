// ssr.comp.hlsl — Screen-Space Reflections compute shader (DX12, Phase 16B)
// Root: SSRCompute
//   b0 = SSRConstants CBV
...
Texture2D<float4>   metalRoughTex : register(t2);
Texture2D<float4>   sceneHDR      : register(t3);
...
cbuffer SSRConstants : register(b0)
{
    ...
    row_major float4x4 proj;         // 64B
    row_major float4x4 invViewProj;  // 64B
    float3   eyePos;       float maxDistance;   // 16B
    uint2    screenSize;   uint maxSteps; float stepSize;  // 16B
    float    thickness;    float maxRoughness; float2 _pad0; // 16B
    float4   _pad1;        // 16B -> 256B total
};
...
{
    // Sample hardware depth, reconstruct view-space Z
...
void main(uint3 tid : SV_DispatchThreadID)
{
    ...
    // Bias origin slightly along ray to avoid self-intersection
