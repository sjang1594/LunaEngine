...
// Root signature bindings — must match DX12Pipeline::CreateRootSignature and shader register assignments.
enum class RootSignatureLayout
{
    MVP,             // b0=MVP CBV
    PBR,             // b0=MVP CBV, b1=MaterialConstants CBV, b2=materialIndex root const (1 DWORD), t0+space1=unbounded SRV heap table, s0=anisotropic sampler
    DeferredLighting,// b0=SceneConstants CBV; params[1]=SRV t0-t4 (GB0/1/2/depth/shadow); params[2]=SRV t5 SSAO; s0=point-clamp
    CSMDepth,        // 16 inline root constants at b0 (light-space MVP 4×4); deny PS
    SSAO,            // b0=SSAOConstants CBV; params[1]=SRV t0 depth; params[2]=SRV t1 normal; params[3]=SRV t2 noise; s0=point-clamp; s1=point-wrap
    SSAOBlur,        // params[0]=SRV t0 raw SSAO; s0=point-clamp
    ...
    BrdfLut,         // compute: u0=brdfLUT RWTexture2D<float2>
...
struct PipelineStateDesc
{
    ...
    bool                depthOnlyPass   = false;  // NumRenderTargets=0, no PS bytecode
