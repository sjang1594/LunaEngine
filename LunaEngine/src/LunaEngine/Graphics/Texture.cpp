...
// Phase 5B: Upload from pre-decoded RGBA8 pixel buffer (embedded GLB textures).
// Mirrors Texture::Load() but skips stbi_load — the caller already has raw pixels.
bool Texture::LoadFromRGBA8(const uint8_t* pixels_rgba8, UINT width, UINT height,
                             ID3D12Device* device,
                             D3D12MA::Allocator* allocator,
                             ID3D12GraphicsCommandList* cmdList,
                             ComPtr<ID3D12Resource>& stagingOut,
                             D3D12MA::Allocation**   stagingAllocOut)
{
    ...
    if (FAILED(hr))
    {
        LUNA_LOG_ERROR("Texture: DEFAULT heap alloc (RGBA8) failed: 0x%08lX", (unsigned long)hr);
        return false;
    }
    ...
    if (FAILED(hr))
    {
        LUNA_LOG_ERROR("Texture: UPLOAD staging alloc (RGBA8) failed: 0x%08lX", (unsigned long)hr);
        return false;
    }
    ...
    for (UINT row = 0; row < _height; ++row)
    {
        const uint8_t* src = pixels_rgba8 + row * _width * bytesPerPixel;
        uint8_t*       dst = static_cast<uint8_t*>(mapped) + row * rowPitch;
        memcpy(dst, src, _width * bytesPerPixel);
    }
    stagingBuf->Unmap(0, nullptr);
