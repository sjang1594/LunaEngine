...
static Luna::Application* s_Instance = nullptr;
...
static VKAPI_ATTR VkBool32 VKAPI_CALL debug_report(VkDebugReportFlagsEXT flags, VkDebugReportObjectTypeEXT objectType, uint64_t object, size_t location, int32_t messageCode, const char* pLayerPrefix, const char* pMessage, void* pUserData)
{
	...
	fprintf(stderr, "[Error]: Debug report from ObjectType: %i\nMessage: %s\n\n", objectType, pMessage);
	return VK_FALSE;
}
...
static void SetupVulkanWindow(ImGui_ImplVulkanH_Window* wd, VkSurfaceKHR surface, int width, int height)
{
	...
	VkPresentModeKHR present_mode[] = { VK_PRESENT_MODE_MAILBOX_KHR, VK_PRESENT_MODE_IMMEDIATE_KHR, VK_PRESENT_MODE_FIFO_KHR };
#else
	VkPresentModeKHR present_mode[] = { VK_PRESENT_MODE_FIFO_KHR };
#endif
	wd->PresentMode = ImGui_ImplVulkanH_SelectPresentMode(g_PhysicalDevice, wd->Surface, &present_mode[0], IM_ARRAYSIZE(present_mode));
...
namespace Luna
{
	Application::Application(const ApplicationSpec& applicationSpec)
		: m_spec(applicationSpec)
	{
		s_Instance = this;
		Init();
	}
	...
	void Application::Init()
	{
		...
		m_WindowsHandle = glfwCreateWindow(m_spec.app_width, m_spec.app_height, m_spec.Name.c_str(), NULL, NULL);
		...
		VkResult err = glfwCreateWindowSurface(g_Instance, m_WindowsHandle, g_Allocator, &surface);
		check_vk_result(err);

		// Create Framebuffers
		int w, h;
		glfwGetFramebufferSize(m_WindowsHandle, &w, &h);
		...
		io.ConfigFlags |= ImGuiConfig;           // Enable Docking
		...
		ImGui_ImplGlfw_InitForVulkan(m_WindowsHandle, true);
